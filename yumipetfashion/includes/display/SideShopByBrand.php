	...
	public function SetPanelSettings()
	{
		...
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			//if($x <= 10) {
				...
				$GLOBALS['QtdeProdutos'] = $row['num'];
				$GLOBALS['ImageFile']    = $GLOBALS['ShopPathNormal'] . "/product_images/" . $row['brandimagefile'];
				$output 		  .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ShopByBrandItem");
				$outputSliderEdaz .= $GLOBALS['ISC_CLASS_TEMPLATE']->GetSnippet("ShopByBrandItemSliderEdaz");
			//}
			++$x;
		}
		...
		$GLOBALS['SNIPPETS']['SideShopByBrandListSliderEdaz'] = $outputSliderEdaz;
	}
}
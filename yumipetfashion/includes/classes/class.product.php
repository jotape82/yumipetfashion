	...
	class ISC_PRODUCT
	{
		...
		public function _SetProductData($productid=0)
		{
			...
			if ($row['opengraph_use_image'] != '') {
				$this->_opengraph_image = GetConfig('ShopPathNormal') . '/' . GetConfig('ImageDirectory') . '/' . $row['imagefile'];
			}
		...
		*/
		public function GetCombinationDetails($combination)
		{
			...
			if($combination['vcimagezoom'] != '') {
				...
				$path = ISC_BASE_PATH . '/' . GetConfig('ImageDirectory') . '/' . $combination['vcimagezoom'];
				try {
					$library = ISC_IMAGE_LIBRARY_FACTORY::getImageLibraryInstance($path);
					$zoomWidth = $library->getWidth();
					$zoomHeight = $library->getHeight();

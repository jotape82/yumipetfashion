	...
	class ISC_CUSTOMER
	{
		...
		*/
		private function CreateAccountStep1($Error = "")
		{
			...
			$phoneNo = '';
			foreach (array_keys($fields) as $fieldId) {
				$fieldPrivateId = $fields[$fieldId]->record['formfieldprivateid'];
				
				/* EDAZCOMMERCE - Setando o atributo no campo para tratamento em tela */
				$fields[$fieldId]->setAttributePrivateId($fieldPrivateId);
				...
				if (isc_strtolower($fieldPrivateId) == 'phone') {
				...
				} else if (isc_strtolower($fieldPrivateId) == 'state') {
				 ...
				 */
				if (isc_strtolower($fieldPrivateId) == 'savethisaddress' || isc_strtolower($fieldPrivateId) == 'shiptoaddress') {
					continue;
				}
		...
		*/
		...
		public function verifyPassword($customer, $password)
		{
			...
			if (empty($customer['salt'])) {
				// backward compatibility for pre 6.0
				// if salt is empty, then use old password check method
				if (md5($password) == $customer['custpassword']) {
					// auto salt old password
					$entity->updatePassword($customer['customerid'], $password);
					return true;
				}
			} else {
				$hash = $entity->generatePasswordHash($password, $customer['salt']);

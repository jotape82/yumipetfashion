	...
	*/
	...
	public function loadProductData(ISC_QUOTE_ITEM $additionalItem = null)
	{
		...
		if (!empty($variationIds)) {
			$query = "
				SELECT
					combinationid, vcproductid, vcimagethumb, vcweight,
	...
	*/
	...
	public function applyCoupon($coupon)
	{
		...
		if (isset($coupon['couponminpurchase']) && $coupon['couponminpurchase'] > 0 && $subtotal < $coupon['couponminpurchase']) {
			throw new ISC_QUOTE_EXCEPTION(GetLang('InvalidCouponMinPrice'), ISC_QUOTE_EXCEPTION::COUPON_MIN_PURCHASE);
		}
		...
		if ($applyOrderDiscount) {
			...
			$runningTotal = $coupon['couponamount'];
			foreach ($this->items as $item) {

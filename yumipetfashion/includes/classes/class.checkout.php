...
class ISC_CHECKOUT
{
	...
	public function ExpressCheckoutChooseAddress($addressType, $buildRequiredJS=false)
	{
		...
		if($addressType == 'billing' && !customerIsSignedIn()) {
			...
			$GLOBALS['CompiledFormFields'] .= $GLOBALS['ISC_CLASS_FORM']->loadFormField(FORMFIELDS_FORM_ACCOUNT, '1');
		}
		...
		foreach($fields as $fieldId => $formField) {
			...
			if (isc_strtolower($formFieldPrivateId) == 'companyname' || isc_strtolower($formFieldPrivateId) == 'country') {
	...
	*/
	...
	public function SavePendingOrder()
	{
		...
		if($verifyPaymentProvider) {
			...
			if ((count($providers) == 0 ||
				(count($providers) > 1 && !isset($_POST['checkout_provider']))) &&
	...
	public static function getQuoteTotalRows(ISC_QUOTE $quote, $displayIncTax = null, $expandShipping = true)
	{
		...
		foreach($coupons as $coupon) {
			...
			$totalRows['coupon-'.$coupon['id']] = array(
				'type' => 'coupon',
			...
			if ($expandShipping) {
				...
				foreach($shippingAddresses as $address) {
					...
					$totalRows['shipping-'.$address->getId()] = array(
						...
						'value' => $address->getShippingCost($displayIncTax)
					);
				}
			}
			else {
				$totalRows['shipping'] = array(
					...
					'value' => $quote->getShippingCost($displayIncTax),
				);
			}
		}

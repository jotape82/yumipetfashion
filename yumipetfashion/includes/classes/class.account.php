	...
	class ISC_ACCOUNT
	{
		...
		*/
		private function Inbox()
		{
			...
			$query = sprintf("select orderid, orddate, total_inc_tax, ordcurrencyid, ordcurrencyexchangerate from [|PREFIX|]orders where ordcustid='%d' and ordstatus > 0 order by orderid desc", $GLOBALS['ISC_CLASS_DB']->Quote($GLOBALS['ISC_CLASS_CUSTOMER']->GetCustomerId()));
			$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		...
		*/
		private function ViewOrderDetails()
		{
			...
			$query = "
				...
				FROM [|PREFIX|]orders
				WHERE ordcustid='".(int)$customerId."' AND orderid='".(int)$orderId."'
			";
			$result = $GLOBALS['ISC_CLASS_DB']->query($query);
			$row = $GLOBALS['ISC_CLASS_DB']->fetch($result);
			if(!$row) {
			...
			if ($viewed != "") {
				...
				if (!empty($viewed_products)) {
					...
					if ($GLOBALS['ISC_CLASS_DB']->CountResult($result) > 0) {
						...
						$productPanelObj = new PRODUCTS_PANEL();
						
						$GLOBALS['AlternateClass'] = '';
						while ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
							$productPanelObj->setProductGlobals($row);

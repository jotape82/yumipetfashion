<?php
	...
	/**
	* This is the Google analytics module for Shopping Cart. To enable
	* this module in Shopping Cart login to the control panel and click the
	...
	class ANALYTICS_GOOGLEANALYTICS extends ISC_ANALYTICS
	{
		...
		*/
		...
		public function GetConversionCode()
		{
			...
			$query = "
				SELECT ordprodid, ordprodvariationid, ordprodsku, total_inc_tax, ordprodname, ordprodqty
			...
			if(strpos($trackingCode, 'urchin.js') === true) {
				...
				while($product = $GLOBALS['ISC_CLASS_DB']->Fetch($productResult)) {
					...
					$trackingPieces = array(
						...
						$product['ordprodname'],
						'',
						number_format($product['total_inc_tax'], 2, '.', ''),
						$product['ordprodqty']
					);
					$conversionCode .= implode('|', $trackingPieces)."\n";
				}

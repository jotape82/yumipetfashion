...
class ACCOUNTING_QUICKBOOKS_ENTITY_CUSTOMERGUEST extends ACCOUNTING_QUICKBOOKS_ENTITY_BASE
{
	public function buildXML()
	{
		if (isc_strtolower($this->spool["service"]) !== "add" && is_array($this->spoolReferenceData)) {
			...
			$query = "SELECT * FROM " . self::refTable . " WHERE accountingreftype='customerguest' AND accountingrefvalue LIKE '%" . $GLOBALS["ISC_CLASS_DB"]->Quote($fullName) . "%' ORDER BY accoutingrefid DESC LIMIT 1";

<?php
	...
	// The absolute filesystem root to Shopping Cart
	...
	// The minimum version of PHP required to run Shopping Cart
	define("PHP_VERSION_REQUIRED", "5.1.4");

	// The minimum version of MySQL required to run Shopping Cart
	define("MYSQL_VERSION_REQUIRED", "4.1.0");

	// What version are we running?
	define('PRODUCT_ID', 'ISC');
	define('PRODUCT_VERSION', '6.0.14');
	define('PRODUCT_VERSION_CODE', 6014);
	...
	if (version_compare(PHP_VERSION, PHP_VERSION_REQUIRED, '<')) {
		die("<h1>PHP ".PHP_VERSION_REQUIRED." or higher is required to run Shopping Cart.</h1>");
	}
	...
	if (GetConfig('isSetup')) {
		...
		/** EDAZCOMMERCE - PATHDAS IMAGENS GERAIS QUE INDEPENDEM DE TEMPLATE **/
		$GLOBALS['IMG_PATH_MASTER'] = GetConfig('ShopPath').'/templates/__master/images';

...
class Interspire_EmailIntegration_Subscription_Customer extends Interspire_EmailIntegration_Subscription
{
	...
	public function __construct($customerId = null)
	{
		...
		// find last used _billing_ address for this customer by orders
		$order = $GLOBALS['ISC_CLASS_DB']->FetchRow("SELECT ordformsessionid, ordbillstreet1, ordbillstreet2, ordbillsuburb, ordbillstate, ordbillzip, ordbillcountryid FROM `[|PREFIX|]orders` WHERE ordcustid = " . (int)$customerId . " ORDER BY orddate DESC LIMIT 1");
		if (is_array($order)) {

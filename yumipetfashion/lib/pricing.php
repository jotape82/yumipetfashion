...
function formatProductPrice($product, $price, array $options = array())
{
	...
	if($product['prodretailprice'] > 0 && $options['strikeRetail'] && $product['prodretailprice'] > $actualPrice ) {
		...
		$output .= '<span class="price_old"><label>De: </label><s>'. FormatPrice($rrp).'</s></span> ';
	}
	...
	if($product['prodsaleprice'] > 0 && $product['prodsaleprice'] < $product['prodprice']) {
		...
		$output .= '<span class="price_current"><label>Por: </label>' .  FormatPrice($actualPrice) . '</span>';
	}
	else {
		...
		if($actualPrice < $retailPrice) {
			$savings = $retailPrice - $actualPrice;
			$output .= "<span class=\"YouSave\">".sprintf(GetLang('YouSave'), "<span class='YouSaveAmount'>" . CurrencyConvertFormatPrice($savings) . "</span>")."</span>";
		}
	}
 ...
 * @return Float
*/
function DefaultPriceFormat($Val, $numberFormat=true)
{
	$regex = "#[^0-9";

...
class ISC_FORMFIELD_SELECTORTEXT extends ISC_FORMFIELD_BASE
{
	...
	public function loadForFrontend()
	{
		...
		if ($this->extraInfo['chooseprefix'] !== '') {
			$selectOptions .= '<option value="">' .  GetLang($this->extraInfo['chooseprefix']) . '</option>' . "\n";
		 ...
		 */
			...
			$formField = '<select name="' . $fieldName . '" id="' . $fieldId . '" class="FormField JSHidden ' . $fieldClass . '" style="display: none; ' . $fieldStyle . ' ' . $this->getFieldStyle() . '" attributePrivateId="' . isc_html_escape(isc_strtolower($this->getAttributePrivateId())) . '" >';

...
class ISC_FORMFIELD_SINGLESELECT extends ISC_FORMFIELD_BASE
{
	...
	public function loadForFrontend()
	{
		...
		if ($this->extraInfo['chooseprefix'] !== '') {
			$GLOBALS['FormFieldOptions'] .= '<option value="">' . GetLang($this->extraInfo['chooseprefix']) . '</option>' . "\n";
		...
		$GLOBALS['FormFieldDefaultArgs'] .= ' id="' . isc_html_escape($this->getFieldId()) . '" name="' . isc_html_escape($this->getFieldName()) . '" attributePrivateId="' . isc_html_escape(isc_strtolower($this->getAttributePrivateId())) . '" style="' . $this->getFieldStyle() . '" ';

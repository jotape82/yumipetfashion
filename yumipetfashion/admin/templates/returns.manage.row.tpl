	<tr id="tr{{ ReturnId|safe }}" class="GridRow" onmouseover="this.className='GridRowOver'" onmouseout="this.className='GridRow'">
		...
		<td class="{{ SortedFieldOrderClass|safe }}">
			<a href="index.php?ToDo=viewOrders&amp;searchQuery={{ OrderId|safe }}">{% lang 'OrderNo' %}{{ OrderId|safe }}</a>
		</td>
		...
		<td class="{{ SortedFieldStatusClass|safe }}">
			<select {{ ReturnStatusDisabled|safe }} name="return_status_{{ ReturnId|safe }}" id="status_{{ ReturnId|safe }}" class="Field" onchange="UpdateReturnStatus({{ ReturnId|safe }}, this.options[this.selectedIndex].value, this.options[this.selectedIndex].text, {{ ReturnStatus|safe }})">

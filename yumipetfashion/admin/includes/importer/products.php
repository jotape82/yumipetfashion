	...
	protected function _ImportRecord($record)
	{
		...
		if (!empty($record['productid'])) {
			...
			if($existing = $GLOBALS["ISC_CLASS_DB"]->Fetch($result)) {
				...
				if($isOverrideDuplicates) {
					...
					// does the product belong to the current vendor?
				...
				$this->ImportSession['Results']['Failures'][] = $record['productid'] . " " . GetLang('ImportProductNotFound');
				return;
			}
		...
		foreach ($priceFields as $field) {
			...
			if (!IsPrice($record[$field])) {
				...
				$this->ImportSession['Results']['Warning'][] = $record['prodname']." ".GetLang('ImportProductInvalidPrice');
			}
		}
		...
		if (empty($exisitng) && ((isset($record['prodlowinv']) && trim($record['prodlowinv']) !== '') || (isset($record['prodcurrentinv']) && trim($record['prodcurrentinv']) !== ''))) {
		...
		if ($record['prodeventdaterequired']) {
			...
			if (empty($record['prodeventdatefieldname'])) {
				...
				$this->ImportSession['Results']['Warnings'][] = $record['prodname'] . ' ' . GetLang('ImportNoEventDateName');
			}
			else {
				...
				if ($record['prodeventdatelimited']) {
					...
					elseif ($record['prodeventdatelimitedenddate'] > $record['prodeventdatelimitedstartdate']) {
						...
						$this->ImportSession['Results']['Warnings'][] = $record['prodname'] . ' ' . GetLang('ImportEventDateInvalid');
					}
				}
			}
		}
	...
	private function _ImportImage($productId, $record, $index = '')
	{
		...
		if (!$existingImage || $existingImage->getSourceFilePath() != $imageFile) {
			if (preg_match('#^(?P<scheme>[a-zA-Z0-9\.]+)://#i', $imageFile, $matches)) {
				...
				$imageAdmin->importImagesFromUrls(false, array($imageFile), $importImages, $importImageErrors);
				...
				if (file_exists($importImageFilePath)) {
					try {
						$importedImage = ISC_PRODUCT_IMAGE::importImage($importImageFilePath, basename($importImageFilePath), false, false, false);
						$productImages[] = $importedImage;

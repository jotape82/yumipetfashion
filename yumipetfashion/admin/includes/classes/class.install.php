<?php
	 ...
	 * KEYGEN
	 *
	 * Generate a valid Key for the ISC5 installation.
	 *
	 * @version     v 1.1
	 * @author      TrioxX
	 * @copyright   Copyright (c) 2010 DecodeThe.Net
	 * @package     Shopping Cart
	 */

	// Replace this value, if you are experincing problems by generating a valid key
	$domain = $_SERVER['SERVER_NAME'];

	// Versions and their keys
	$versions = array("Starter"=>"1","Professional"=>"2", "Ultimate"=>"4","Vendor"=>"8");

	// Create the key
	if(isset($_POST['myversion'])){
		$myversion = $_POST['myversion'];
		$_POST['LK'] = 'ISC'.base64_encode(pack("CCVvvH*", 16,$versions[$myversion],0,0,0,md5($domain)));
	}
...
//			}
			...
			if($connection) {
				...
				if($mysql_check < 0) {
					echo "ERRO: " . $GLOBALS["ISC_CLASS_DB"]->Error() . '\n';

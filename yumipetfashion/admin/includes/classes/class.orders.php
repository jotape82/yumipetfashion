<?php
	class ISC_ADMIN_ORDERS extends ISC_ADMIN_BASE
	{
		...
		*/
		...
		protected function DeleteOrders()
		{
			...
			if($vendorId > 0) {
				...
				if (!$result) {
					$this->ManageOrders($GLOBALS['ISC_CLASS_DB']->GetErrorMsg(), MSG_ERROR);
					return;
				}
			...
			// use entities to delete order so related data is correctly deleted
			foreach ($orderIds as $orderId) {
				$entity = new ISC_ENTITY_ORDER;
				if (!$entity->delete($orderId)) {
		...
		protected function ManageOrdersGrid(&$numOrders)
		{
			...
			$orderResult = $this->_GetOrderList($start, $sortField, $sortOrder, $numOrders, $perPage);
		...
		*/
		public function _GetOrderList($Start, $SortField, $SortOrder, &$NumOrders, $limit = ISC_ORDERS_PER_PAGE)
		{
			$extraFields = '';
			$extraJoins = '';

	...
	protected function _handleAuctionCheckoutComplete($body)
	{
		...
		if (!empty ($body['Item']['ItemID']) && ISC_ADMIN_EBAY::validEbayItemId($body['Item']['ItemID'])) {
			...
			if (!empty($body['TransactionArray']['Transaction']['ContainingOrder'])) {
				...
				if (GetOrderByEbayOrderId($orderId)) {
					return true;
				}
				...
				$eachItemPriceExTax = $body['TransactionArray']['Transaction']['TransactionPrice']['!'];
				$quantityPurchased = $body['TransactionArray']['Transaction']['QuantityPurchased'];
				$totalTransactionPrice = $quantityPurchased * $eachItemPriceExTax;
				$variationOptionsString = '';

				// check if the order already exist for the ebay product.
				$query = "
					SELECT *
					FROM [|PREFIX|]order_products
					WHERE ebay_item_id = '".$body['Item']['ItemID']."'
					LIMIT 1
				";
				$res = $GLOBALS['ISC_CLASS_DB']->Query($query);
				if ($GLOBALS['ISC_CLASS_DB']->Fetch($res)) {
					return true;
				}

				// do we have a variation for this product?
				if (isset($body['TransactionArray']['Transaction']['Variation']['VariationSpecifics'])) {
					$variationNameValueList = $body['TransactionArray']['Transaction']['Variation']['VariationSpecifics']['NameValueList'];
					$variationOptions = array();
				...
				$order['ShippingCost'] = $body['TransactionArray']['Transaction']['ShippingServiceSelected']['ShippingServiceCost']['!'];
				if (!isset ($body['TransactionArray']['Transaction']['ShippingServiceSelected']['ShippingInsuranceCost']['!'])) {
					$order['ShippingInsuranceCost'] = $body['TransactionArray']['Transaction']['ShippingServiceSelected']['ShippingInsuranceCost']['!'];
				}
				$order['GrandTotal'] = $body['TransactionArray']['Transaction']['AmountPaid']['!'];
				$order['Transaction'][] = array(
					'QuantityPurchased' => $quantityPurchased,
			...
			$newOrder = array(
				...
				'ordipaddress' => GetIP(), // @dc_todo: testing it
				'ordcustmessage' => '',
				'ordstatus' => ORDER_STATUS_AWAITING_FULFILLMENT,
			...
			foreach ($order['Transaction'] as $eachTransaction) {
				...
				// Link the order id to ebay item in the store

...
class ISC_ADMIN_INDEX extends ISC_ADMIN_BASE
{
	...
	public function ShowDashboard()
	{
		...
		$l['expires'] = 0; /** EDAZCOMMERCE - VERSION LICENSED **/
	...
	public function GenerateOrderBreakdownGraph()
	{
		...
		$arrayDiasDaSemana = array("Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado");
		...
		for($i=$graphFrom; $i <= time(); $i += 86400) {
			...
			if($statsDate == $todaysDate) {
				...
				$label = intval(isc_date('w', $i));
			}
			
			$label = $arrayDiasDaSemana[$label];
